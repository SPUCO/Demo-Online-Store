...
if(isset($_POST['search']) && isset($_POST['type'])){
  ...
  $stmt = $conn->prepare("SELECT COUNT(*) AS fldtotalrecords FROM products WHERE fldproducttype = ? AND fldproductprice <= ?");
  ...
  //4. get all products
  $stmt1 = $conn->prepare("SELECT * FROM products WHERE fldproducttype = ? AND fldproductprice <= ? LIMIT $offset,$totalrecordsperpage");
  $stmt1->bind_param("ss",$type,$price);
  $stmt1->execute();
